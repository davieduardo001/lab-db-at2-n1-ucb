...
CREATE TABLE IF NOT EXISTS pais (
  ...
  `continente_codcontinente` VARCHAR(10) NOT NULL,  
  PRIMARY KEY (`codPais`),
...
CREATE TABLE IF NOT EXISTS geracaoEnergiaPessoaPais (
  ...
  INDEX `fk_geracaoEnergiaPessoaPais_Pais_idx` (`Pais_codPais` ASC),
  CONSTRAINT `fk_geracaoEnergiaPessoaPais_Pais`
    FOREIGN KEY (`Pais_codPais`)
    REFERENCES `trabalho`.`Pais` (`codPais`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION
...
CREATE TABLE IF NOT EXISTS percentPopEnergiaLimpa (
  ...
  CONSTRAINT `fk_percentPopEnergiaLimpa_Pais1`
    FOREIGN KEY (`Pais_codPais`)
    REFERENCES `trabalho`.`pais` (`codPais`) -- Certifique-se de referenciar o banco correto
    ON DELETE NO ACTION
    ON UPDATE NO ACTION
);